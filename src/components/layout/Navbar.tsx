...
import { Sparkles, Menu, X, Globe, HeartPulse } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ currentLang: propLang, onSelectLang }) => {
  const location = useLocation();
  const { currentLang: contextLang, setLanguage, t } = useI18n();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [langDropdownOpen, setLangDropdownOpen] = useState(false);

  const activeLangCode = propLang || contextLang;
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 font-medium hidden sm:block">{t('nav.subtitle')}</p>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-white/90 border border-slate-200/90 text-sm font-semibold text-slate-700 hover:bg-slate-50 shadow-sm transition-all cursor-pointer"
              >
              ...
              {langDropdownOpen && (
                <div className="absolute right-0 mt-2 w-52 bg-white/95 rounded-2xl shadow-xl border border-slate-200/90 py-2 z-50 backdrop-blur-lg animate-in fade-in duration-150">
                  <div className="px-3 py-1 mb-1 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                  ...
                  {SUPPORTED_LANGUAGES.map((lang) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-2 text-sm text-left font-medium transition-colors cursor-pointer ${
        ...
        <div className="md:hidden glass-card mx-4 mb-4 p-4 border border-slate-200/80 rounded-2xl shadow-xl animate-in slide-in-from-top-4 duration-200">
          <div className="flex flex-col gap-2">
            ...
            <div className="mt-2 pt-2 border-t border-slate-200">
              <p className="px-4 text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">{t('nav.selectLanguage')}</p>
